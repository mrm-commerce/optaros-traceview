...
class Optaros_TraceView_Model_Observer {
	...
	public function preDispatch($observer) {
		...
		/* log request info */
		oboe_log('mage_routers_match', 'info', 
			array(
		...
		{
		    oboe_log('mage_routers_match', 'info', array('Partition' => 'Admin'));

		}
		else if ($session && ($session->getCustomerId() !== NULL)) {
			oboe_log('mage_routers_match', 'info', array('Partition' => 'LoggedIn'));
		} else {
			oboe_log('mage_routers_match', 'info', array('Partition' => 'Anonymous'));
		}
	}
}
