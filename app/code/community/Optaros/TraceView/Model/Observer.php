...
class Optaros_TraceView_Model_Observer {
	...
	public function preDispatch($observer) {
		...
		{
		    oboe_log('magento', 'info', array('Partition' => 'Admin'));

		}
		else if ($session && ($session->getCustomerId() !== NULL)) {
			oboe_log('magento', 'info', array('Partition' => 'LoggedIn'));
		} else {
			oboe_log('magento', 'info', array('Partition' => 'Anonymous'));
		}
	}
}
